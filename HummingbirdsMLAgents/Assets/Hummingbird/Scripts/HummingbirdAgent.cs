...
public class HummingbirdAgent : Agent
{
    ...
    public float[] DistanceTracker = new float[20];

    public int DistanceTrackerCount = 0;
    public int DistanceTrackerCountThreshold = 20;

    public void Start()
    {
        for (int idx = 0; idx < DistanceTracker.Length; idx++)
        {
            DistanceTracker[idx] = int.MaxValue;
        }
    }
    ...
    {
        // Reward if bird has been going toward a flower for a continuous space of time
        bool reward = true;
        if (DistanceTrackerCount > DistanceTrackerCountThreshold)
        {
            Vector3 toFlower = nearestFlower.FlowerCenterPosition - beakTip.position;
            var currentDistance = Vector3.Dot(toFlower.normalized, -nearestFlower.FlowerUpVector.normalized);
            for (int idx = 0; idx < DistanceTracker.Length; idx++)
            {
                if (currentDistance > DistanceTracker[idx] || DistanceTracker[idx] == int.MaxValue)
                {
                    reward = false;
                }
            }

            for (int idx = 0; idx < DistanceTracker.Length - 1; idx++)
            {
                DistanceTracker[idx] = DistanceTracker[idx + 1];
            }

            DistanceTracker[DistanceTracker.Length - 1] = currentDistance;
            ...
            DistanceTrackerCount = 0;
        }

        DistanceTrackerCount++;
